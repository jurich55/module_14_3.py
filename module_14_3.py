...
async def get_buying_list(message):
    for i in range(1, 5):
        product_messages = (f'Название: Product{i} |'
                            f' Описание: Комплект {i} | Цена: {i * 100}')
        await message.answer(product_messages)  # Отправляем product_messages
        with open(f'files/{i}.png', 'rb') as png:
            await message.answer_photo(png)         # Отправляем  фото  комплекта
    await message.answer('Выберите продукт для покупки:',
            reply_markup=catalog_kb)                   #  вызов  клавиатуры catalog_kb
